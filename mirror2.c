...
// defining constants as per the requirements
#define serPortCon 6312
#define mirror2PortCon 6314
#define maxClient 20
...
// using this function we are sending the tar file to the client
// we are sending the tar file in chunks of 1024 bytes
// sending message to the client if the file is not found
...
// using this function we are executing the command using fork and exec system calls
...
// in this function we are traversing through the home directory and listing all the directories
// dirent structure is used to read the directory entries in the tree
...
void performW24fz(int clientSocket, const char *parsedCmd)
{
    ...
    else
    {
        ...
        dup2(clientSocket, STDOUT_FILENO);
        // array of extensions
        const char *extnArr[] = {"txt", "xml", "sh", "jpeg", "cpp", "db"
                                                                    "pdf",
                                 "c", "jpg", "png", "jpeg"};
...
int main() {
    ...
    if (listen(mirror2Socket, maxClient) == -1) {
        // print error message if listening failed
        printf("Sorry! Listening Failed\n");
        exit(EXIT_FAILURE);
    }
    // print the success message
    printf("Mirror2 Established Successfully and waiting for clinet request on port: %d...\n", mirror2PortCon);

    while (1) {

        // print the prompt
        printf("\n");
        printf("\033[1;32mmirror1w24$ \033[0m ");

        // accept the client request
        if ((clientSocket = accept(mirror2Socket, (struct sockaddr *)&clientAddrPort, &clientAddrPortSize)) == -1) {
            // print error message if accepting the client request failed
            printf("Sorry! Failed to Accept the Client's Connection\n");
            continue;
        }

        // recieve the client count from the client

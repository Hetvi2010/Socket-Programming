...
int main() {
    ...
    int mirror1Socket;
    int clientSocket;
    int isUsed = 1;  // Set the SO_REUSEADDR option
    int clientCnt = 0;
    struct sockaddr_in mirror1AddrPort;
    struct sockaddr_in clientAddrPort;
    socklen_t clientAddrPortSize = sizeof(clientAddrPort);
   

    // create socket
    if ((mirror1Socket = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
        // print error message if socket creation failed
        printf("Sorry! Socket Creation Failed\n");
        exit(EXIT_FAILURE);
    }

    if (setsockopt(mirror1Socket, SOL_SOCKET, SO_REUSEADDR, &isUsed, sizeof(int)) == -1) {
        // print error message if setsockopt failed
        printf("Sorry! setsockopt Failed\n");
        exit(EXIT_FAILURE);
    }

    // set the address and port for mirror1
    mirror1AddrPort.sin_family = AF_INET;
    mirror1AddrPort.sin_port = htons(mirror1PortCon);
    mirror1AddrPort.sin_addr.s_addr = INADDR_ANY;

    // bind the server socket to the address and port
    if (bind(mirror1Socket, (struct sockaddr *)&mirror1AddrPort, sizeof(mirror1AddrPort)) == -1) {
    ...
    if (listen(mirror1Socket, maxClient) == -1) {
        // print error message if listening failed
        printf("Sorry! Listening Failed\n");
        exit(EXIT_FAILURE);
    }
    // print the success message
    printf("Mirror1 Established Successfully and waiting for clinet request on port: %d...\n", mirror1PortCon);
    ...
    while (1) {
        ...
        if ((clientSocket = accept(mirror1Socket, (struct sockaddr *)&clientAddrPort, &clientAddrPortSize)) == -1) {
